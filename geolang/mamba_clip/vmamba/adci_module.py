...
class ADCI(nn.Module):
    def __init__(self, in_channels_list, align_channels, out_channels, G, target_size):
        """
        ...
        assert len(in_channels_list) % G == 0, "Number of layers L must be exactly divisible by number of groups G."
...
def test_adci():
    ...
    for i in range(len(C_list)):
        features.append(torch.randn(B, C_list[i], H_list[i], W_list[i], requires_grad=True))
    ...
    for i, f in enumerate(features):\
        assert f.grad is not None, f"Gradients did not flow to input feature {i}"
